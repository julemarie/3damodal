...
orig_state_dict = torch.load("3DAmodal/checkpoint_orig/aisformer_r50_kins.pth", map_location='cpu')
...
class Backbone(nn.Module):
    def __init__(self, cfg):
        ...
        self.bb_threshold = self.cfg.MODEL.BACKBONE.BB_THRESH
        ...
        self.init_fpn(orig_state_dict)


    def init_fpn(self, state_dict):
        with torch.no_grad():
    ...
    def forward(self, img):
        ...
        return final_features, bbs




class AISFormer_orig(nn.Module):
    def __init__(self, devices, cfg):
        super(AISFormer_orig, self).__init__()
        # fmt: off
...
def test():
    ...
    devices = ['cpu', 'cpu']
    model = AISFormer_orig(devices, cfg)
...
def test2():
    ...
    for key in state_dict['model'].keys():
        if "mask_head_model" in key:
            if "predictor" not in key:
                new_state_dict[key.replace("roi_heads.mask_head.mask_head_model.", "")] = state_dict['model'][key]
    # model.to(device='cuda')
    aisformer.load_state_dict(new_state_dict)

    x = {'image': img, 'lidar': None}
    ...
    save_image(a_masks[0], "a_masks.png")
    save_image(invisible_masks[0], "invisible_masks.png")
